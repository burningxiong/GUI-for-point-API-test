...
class MyFrame():
	...
	def __init__(self):
		...
		# 设置主窗口标题
		self.master.wm_title('积分接口测试v0.3')
		# 设置窗口大小
	...
	def submit(self):
		...
		if '设置' == self.vChoose.get():
			if '' != self.vHost.get() and '' != self.vPort.get() and '' != self.vAppid.get() and '' != self.vSecretKey.get():
				try:
					httplib.HTTPConnection(self.vHost.get()+':'+self.vPort.get(),timeout=5)
				except:
	...
	def clearAll(self):
		for i in [self.vUsername,self.vPort,self.vHost,self.vNumbers,self.vPage,self.vAmount,self.vCause,self.vAppid,self.vEndTime
			...
			self.vSecretKey,self.vinTradeno]:
			try:
				i.set('')
			except:
				pass
	...
	def showResult(self,res,interval,body,reqtype):
		if '200' == res:
			...
			for i in [402,403,404,405,500,501,601,602,603,604,605,606]:
				if i == int(res):
	...
	def sendmsg_http(self,str_msg_http,str_host_ip,str_host_port):
		# 必须每次新建http连接，此处无法复用
		try:
			obj_conn = httplib.HTTPConnection(str_host_ip+':'+str_host_port,timeout=5)
		except Exception, e:
			obj_conn.close()
			return e,'xxxx'
		# 发送请求
		try:
			int_st = time.time()
			obj_conn.request('POST',str_msg_http)
		except Exception, e:
			obj_conn.close()
			return e,'xxxx'
		# 获取响应
